...
- **A live auto queue** — lint what's actually deployed:
  ```bash
  python ~/.claude/bos-run.py tool lint-sequence --queue <auto_queue_id> --json
  ```
  (Get the queue id from `/nurture-health` or `python ~/.claude/bos-run.py tool dump-crm-bundle --resources auto_queues`.)

- **Local drafts** — lint before shipping, e.g. the drafts from
  `design-nurture-sequence`:
  ```bash
  python ~/.claude/bos-run.py tool lint-sequence --drafts drafts.json --json
  ```
