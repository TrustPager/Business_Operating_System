...
| **Break down how a channel grew** | Point me at one YouTube channel and I'll study how it grew: a performance timeline across its uploads, the moment its views durably stepped up (or the spike that faded), what changed at that point, and the one move you can borrow. Every number real and observed, and it runs on your own machine. | `break-down-a-channel` `[live]` | keyless | deeper |
...
| **See what worked on your own channel** | Once you've published, I'll pull your own uploads on your machine, score each one against your own recent baseline, and tell you what was different about the ones that moved (the idea, the packaging, the hook, the format, the pacing) plus the one thing to repeat in your next video. Every number real and observed. | `what-worked` `[live]`, `package-my-video` `[live]` | keyless | deeper |
| **Make a professional video on your own brand** | Give me a topic or a script and I'll make a real motion-graphics video on your machine: I plan the visuals so every beat shows one clear idea (a before-and-after, a step flow, one big number), render a quick draft you watch, then we refine it one change at a time until it's right. On your palette and your voice, no accounts and no editing skills needed. | `make-my-video` `[live]`, `design-my-scenes` `[live]`, `script-my-video` `[live]` | keyless | deeper |
...
3. **Anchor on the named relief.** The first of the 3 must map to `named_relief`, and cold it must be a **live keyless** win (the one Planned project below is named for vision only — never lead with it until it ships). Relief → project lead:
   ...
   - finding leads → **zero customers / no pipeline yet (pre-launch or just-starting)?** lead with *Draft your first-client outreach* (**drafted live in chat, not an app** — the same inline pattern as start-here's chase-quotes entry, so it never trips the routing allow-list: 10-20 personalised messages to people they already know, the §10.1 warm-outreach pattern with a "who do you know who…?" ask, per `knowledge/business-method.md` §16's new-business row and the §2 pre-launch rule; if the recommendation needs a registry-backed build behind it, pair it with *Your brand, written down* — every other option on this row assumes an existing site, customers, or a live market, so never lead a zero-client owner with them) · **otherwise** → *See why you're not getting found* (live keyless, online — for a local/service business this is often the best-fit lead, and it self-enforces the §10.5 gate by ordering answer-speed + reviews ahead of keywords) · *Set up a referral engine that actually asks* (live keyless — word-of-mouth is the cheapest lead source, §10.7) · *Size up a competitor* (live keyless, online) · *Research a prospect* (live keyless, online); fall back to *Your brand, written down* (live) if offline
