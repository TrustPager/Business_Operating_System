...
**Build-status legend:** `[live]` = ships today (in registry, `status: active`); `[floor-new]` = decided keyless floor build, not yet built; `[fix]` = exists but needs the keyless paste/local-file path (only `transcript-summary`).
...
|---|---|---|---|---|
| **Price this job with confidence** | Tell me the work, your costs and hours — I'll build a defensible price with margin shown, so you never quote off a gut feel again. | `price-my-work` `[floor-new]`, `build-spreadsheet` `[live]` | keyless | first_build |
| **Turn a photo into a ready-to-send quote** | Send a photo of the job and a quick voice note — I'll turn it into a laid-out quote with line items and a price you can stand behind. | `quote-from-photo` `[live]`, `price-my-work` `[floor-new]` | keyless | instant_win |
| **Write the proposal that wins the job** | Give me the scope and price and I'll write a sharp, on-brand proposal/SOW in your voice, ready to send. | `write-a-proposal` `[floor-new]`, `price-my-work` `[floor-new]`, `build-brand-strategy` `[live]` | keyless | first_build |
| **Size up a competitor before you bid** | Point me at a rival's site and I'll read it like a sharp operator — how they position, what they charge, where the gaps are. | `research-a-competitor` `[floor-new]` | keyless | first_build |
| **Research a prospect/partner before the call** | Tell me who you're meeting and I'll hand you a one-page brief plus three sharp questions that make you the most prepared person in the room. | `research-before-call` `[floor-new]`, `research-a-competitor` `[floor-new]` | keyless | instant_win |
| **Build your pricing brain** | Tell me how you price your common jobs and I'll build a sheet you can quote off in seconds — consistent every time. | `price-my-work` `[floor-new]`, `build-spreadsheet` `[live]` | keyless | first_build |
...
| Project | One-line operator pitch | Builds on | Keyless/CRM | Tier |
|---|---|---|---|---|
| **Your brand, written down (positioning + voice)** | I'll turn your brain-dump into who you're for, what makes you the obvious choice, and exactly how you sound — then lock it into everything we make. | `build-brand-strategy` `[live]`, `build-customer-voice` `[live]`, `brand-my-workspace` `[live]` | keyless | first_build |
| **A branded post you can publish today** | Give me one thing you want to say this week and I'll make the on-brand graphic plus the caption — ready to drop straight on socials. | `make-social-post` `[live]`, `write-post-copy` `[floor-new]` | keyless | instant_win |
| **A fortnight of content, planned and written** | I'll turn your brand into a two-week plan — what to post, when, where — then write the captions and make the first graphics. | `plan-my-content` `[floor-new]`, `write-post-copy` `[floor-new]`, `make-social-post` `[live]` | keyless | deeper |
| **A short branded promo video** | A 15-30s branded video — your work, your colours, text on screen — the kind of thing that stops the scroll. No film crew. | `make-brand-video` `[floor-new]`, `build-brand-strategy` `[live]` | keyless | deeper |
| **The policies & FAQs your business runs on** | Tell me how you handle deposits, cancellations, privacy — I'll write clean, on-brand policies and FAQ answers for your site, emails or staff. | `write-a-policy` `[floor-new]`, `build-knowledge-base-from-docs` `[live]` | better_with_crm | deeper |
| **A capabilities / explainer one-pager** | I'll write the one clean on-brand answer to "can you do this?" — services, lead times, how it works — ready to send every time. | `write-a-policy` `[floor-new]`, `write-post-copy` `[floor-new]` | keyless | deeper |

### 🗂️ Save time on admin — read any file, forms, structure data

| Project | One-line operator pitch | Builds on | Keyless/CRM | Tier |
|---|---|---|---|---|
| **Throw me any file — I'll read it and structure it** | Messy PDF, supplier spreadsheet, scanned form, contract — send it over and I'll pull out the important bits as clean, usable data. | `extract-document` `[live]`, `compare-documents` `[live]`, `import-from-anywhere` `[live]` | keyless | instant_win |
...
| **A reusable intake / RFQ form, drafted clean** | Hand me your current form or describe it and I'll turn it into a clean structured form spec with the right fields — ready to go live on connect. | `template-from-document` `[live]`, `build-form` `[live]` | keyless | first_build |
...
| Project | One-line operator pitch | Builds on | Keyless/CRM | Tier |
|---|---|---|---|---|
| **Stress-test your next big decision** | A hire, a price rise, dropping a service? Tell me the decision and I'll grill you on it — poke the holes before you commit, not after. | `grill-me-on-this-decision` `[floor-new]`, `write-prompt` `[live]` | keyless | first_build |
| **Hire and onboard your next team member** | I'll write the job ad and screening questions in your voice, then build the day-one onboarding so a new starter sounds like you from week one. | `write-a-job-ad` `[floor-new]`, `onboard-team-member` `[live]`, `sync-team-standards` `[live]` | keyless | deeper |
| **Your process / compliance playbook** | Tell me how you actually run things and I'll write it as a clean playbook and client-facing FAQ — consistent across you and any teammate; powers your assistant on connect. | `write-a-policy` `[floor-new]`, `build-knowledge-base-from-docs` `[live]`, `onboard-team-member` `[live]` | better_with_crm | deeper |

---

## 2. The universal core (the safe default pool)

These ~11 work for **any** 2-10 person AU service business and are the default pool to draw the 3 from when the vertical is unclear or generic. **All but the CRM-tagged ones are keyless**, so they survive a cold open.

1. **Your brand, written down** — `build-brand-strategy` + `build-customer-voice` + `brand-my-workspace` · keyless · first_build *(the spine — writes `brand.json`, reskins every later artifact)*
2. **A branded post you can publish today** — `make-social-post` + `write-post-copy` · keyless · instant_win
3. **Price this job with confidence** — `price-my-work` + `build-spreadsheet` · keyless · first_build
4. **Write the proposal that wins the job** — `write-a-proposal` + `price-my-work` · keyless · first_build
5. **Size up a competitor before you bid** — `research-a-competitor` · keyless · first_build
6. **Research a prospect before the call** — `research-before-call` · keyless · instant_win
7. **Throw me any file — I'll read it and structure it** — `extract-document` + `compare-documents` + `import-from-anywhere` · keyless · instant_win
8. **Turn a call into notes, decisions and next steps** — `transcript-summary` `[fix]` · better_with_crm · instant_win
9. **Stress-test your next big decision** — `grill-me-on-this-decision` + `write-prompt` · keyless · first_build
10. **A fortnight of content, planned and written** — `plan-my-content` + `write-post-copy` + `make-social-post` · keyless · deeper
11. **Hire and onboard your next team member** — `write-a-job-ad` + `onboard-team-member` + `sync-team-standards` · keyless · deeper
12. **Set the policies & FAQs your business runs on** — `write-a-policy` + `build-knowledge-base-from-docs` · better_with_crm · deeper

**Note for builders:** every copy-producing app (`write-post-copy`, `plan-my-content`, `write-a-proposal`, `write-a-job-ad`, `write-a-policy`) must enforce the positive-only language rule (global CLAUDE.md + floor-completion-plan DoD). `transcript-summary` is tagged `better_with_crm` not keyless until the planned MarkItDown paste-path lands (floor-completion-plan ruling #2).
...
- **Renewal radar** (`follow-up-radar` + `prep-for-call`) — for insurance, *renewals are the business*; surfacing them early is the most valuable follow-up.
...
- **A calm reminder & rebooking sequence** (`design-nurture-sequence`) — reminders are the highest-value automation (no-show reduction); logistics-only, never clinical over text.
- **Plain-English funding explainers** (`write-a-policy`) — NDIS/Medicare/private-health wording, confirmed before drafting.

**Consulting & professional services**
- **Stalled-proposal follow-up radar** (`follow-up-radar`) — "proposals stall in sent" is the named #1 gotcha.
- **A fortnight of thought-leadership content** (`plan-my-content` + `write-post-copy`) — being the obvious expert is how referrals start.
- **Retainer renewal & upsell radar** (`sweep-my-day` + `follow-up-radar` + `weekly-review`) — retainers are recurring value; the renewal/upsell moment is the most valuable date.

**Small manufacturing / product**
- **Your spec-to-price calculator** (`price-my-work` + `build-spreadsheet`) — quotes hinge on specs; a calculator turns any spec into a defensible number.
- **A reusable RFQ intake sheet** (`build-form` + `build-spreadsheet`) — missing spec inputs stall every quote.
- **A reorder reminder system** (`design-nurture-sequence` + `follow-up-radar`) — repeat/wholesale accounts are the backbone; reorder timing is the lifeblood.
...
3. **Anchor on the named relief.** The first of the 3 must map to `named_relief`. Relief → project lead:
   - quoting → *Price this job* / *photo-to-quote* / vertical pricing build
   - finding leads → *Size up a competitor* / *Research a prospect*
   - chasing invoices → (cold) *Write the proposal* or *Chase stale quotes*; (connected) *Spot uninvoiced jobs* / *follow-up radar*
   - looking professional → *Your brand, written down* / *A proposal that wins*
   - staying on top of customers → *Throw me any file* / *recovery text* / *call-to-notes*
   - content/marketing → *A branded post today* / *fortnight of content*
   - thinking/deciding → *Stress-test your decision*
...
> 2. **Build your pricing brain** so every quote comes out consistent in seconds.
...
- **instant_win** (the <2-min pre-pivot taste — *not* offered in the 3-menu): *photo-to-quote* on one photo, one *branded post*, *throw-me-any-file*, *research-before-call* on one name, *call-to-notes*. These trigger the pivot; the menu is drawn from the tiers below.
- **first_build** (the cold 3-option pool — keyless, visibly operator-grade, deepens profile as a side effect): *Your brand written down*, *Price this job*, *Build pricing brain*, *Write the proposal*, *Size up a competitor*, *Stress-test a decision*, *reusable intake/RFQ form*, plus the vertical first_builds (*recovery text*, *chase stale quotes*, *outstanding-docs chaser*).
- **deeper** (offered once trust is high or a connection exists): *fortnight of content*, *branded promo video*, *hire & onboard*, *policies/knowledge base*, *customer-list import*, *spreadsheet*, *uninvoiced-jobs*, *renewal/reorder radar*, *nurture sequences*.
