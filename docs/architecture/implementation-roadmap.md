...
**Status (updated 2026-06-29): P0 ✅, P1 ✅, P3 ✅, the keyless floor (Wave 0 + Wave 1) ✅, a 10-persona field test ✅, Wave 1.5 pre-ship hardening ✅, and P5 MONEY + people-ops ✅ — all shipped to `main`.** (P0 = kernel/driver split; P1 = manifest + registry contract; P3 = `start-here` onboarding + `whats-possible` catalog; Wave 0 = the onboarding-binding guardrail + drift repair + the floor roster as definition-of-done; Wave 1 = the full keyless wins library — doc-lib-set/markitdown/firecrawl drivers + 14 apps; Wave 1.5 = cold-install self-sufficiency (D11), inclusive business-shapes (D12), lean-floor-plus-library + social-win-is-a-strategy (D13), the `write_docx` table block, `write-a-letter`/`describe-a-product`/`build-social-strategy`, content-rule guards, and a polish pass; P5 = the `requires_region` gate + binding assertion D, `finance_calc` (numpy-financial), the FY-versioned AU constants module, the universal money apps `profit-per-job`/`cash-flow-forecast`/`renewal-tracker`, and AU-gated `estimate-my-bas` behind an explicit `Region: AU` confirmation. **77 apps registered; 231 tests; binding check green.**)
...
**Shipped since the last status:** the **mechanical em-dash guard** (post-write normalizer on the doc/content output path), **P5 MONEY + people-ops** (see the P5 section), **Public-release readiness (Track 1)** — the repo is now honest, installable, and OSS-ready as a public GitHub project — and **Claude setup best-practices (Track 2)**, the first opt-in "module as course" (see the Track 2 entry below).

**Public-release readiness (Track 1) ✅ shipped to `main` (2026-06-29).** Spec: [`2026-06-29-public-release-readiness-design.md`](2026-06-29-public-release-readiness-design.md); plan: [`plans/2026-06-29-public-release-readiness.md`](plans/2026-06-29-public-release-readiness.md). Delivered: (1) **Cross-OS install that actually works** — the keyless tools run via the proven `~/.claude/bos-run.py` signpost (`run.py` gained a `tool` mode with an allowlist; `setup.py` skips the key cleanly and copies skills into `~/.claude/skills/` for auto-discovery; a cold-install smoke proves clone→setup→tool-run from a foreign cwd). The `${CLAUDE_PLUGIN_ROOT}`-in-skill-body approach was investigated and rejected (does not expand in PowerShell; not documented for skill bodies) in favour of the signpost. (2) **Keyless-first repositioning** of README/INSTALL/plugin-manifests/`commands/` with TrustPager as the positive optional upgrade (pulls P7's user-facing prose de-brand forward); CAPABILITIES audited so heavy render studios are not advertised as cold wins; version `1.0.0`. (3) **OSS files**: CONTRIBUTING, SECURITY, issue/PR templates, CODE_OF_CONDUCT, CHANGELOG. All shipped surfaces are em-dash-clean; git history confirmed secret-free. **The one remaining action is a human one: flipping the GitHub repo to public** (intentionally not automated).
...
- **P2 safety hardening** + **P3 connect-on-demand remainder** (`connect-a-tool` + `tools/preflight.py`, which rides on P2's safety work).
- **P5 connected-tier deepeners** (named, not built): the Fair Work MAPD API and ABS Indicator API as live refresh for the AU pack, gated behind `Region: AU` + a connection; ride on the P7/P8 connection design.
- **CLAUDE.md best-practices onboarding (Track 2) ✅ shipped to `main` (2026-06-29).** Spec: [`2026-06-29-claude-setup-best-practices-design.md`](2026-06-29-claude-setup-best-practices-design.md); plan: [`plans/2026-06-29-claude-setup-best-practices.md`](plans/2026-06-29-claude-setup-best-practices.md). An opt-in keyless module (`tune-my-setup`, NOT routed from start-here) that sets up the user's GLOBAL `~/.claude/CLAUDE.md` best-practices block + merges the recommended `~/.claude/settings.json` permissions, in either a recommended-one-step or guided-Q&A mode, additive and reversible (delimited BOS block, defended against clobbering a malformed/existing file), teaching the why as it goes. Built on `tools/setup_claude_config.py` (allowlisted, temp-home tested). It is the first instance of the **"BOS module as the course, Skool as the pointer"** model: Skool just says "ask your BOS to set up your Claude settings best practices," and the learning happens by doing inside Claude.
- **P7/P8 residue** after Track 1 pulled their user-facing parts forward: P7's deeper re-slot (move `templates/CLAUDE.md` "About TrustPager" → `drivers/trustpager/about.md`, wire Xero as exemplar driver #2); P8's full seam-retirement (`bos-run.py`/`bos.json` is carried forward, not retired) + migration tooling. Lower priority now that the public install works.
- **Pre-public-push checklist** (human action): flip GitHub visibility to public; re-run the git-history secret scan as the gate immediately before.
- **Noted cleanup:** em dashes remain in dev/internal files (SKILL.md bodies, `agents/`, `drivers/`, `tools/*.py`, architecture docs); shipped customer-facing surfaces are clean. Sweep the skill bodies if/when brand-consistency there matters.
