...
**Shipped since the last status:** the **mechanical em-dash guard** (post-write normalizer on the doc/content output path), **P5 MONEY + people-ops** (see the P5 section), and **Public-release readiness (Track 1)** — the repo is now honest, installable, and OSS-ready as a public GitHub project.
...
- **CLAUDE.md best-practices onboarding (Track 2):** a track inside BOS that helps users set up a strong global + project-level `CLAUDE.md` as part of onboarding, promoting best practices to every user. Its own spec, next.
