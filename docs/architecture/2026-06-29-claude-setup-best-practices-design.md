...
A keyless skill: `function_slot: floor` (or `strategy`), `requires_driver: none`, `requires_credential: none`, `data_path: local`. **`data_path` note (resolves spec-review Issue 1):** the existing `local` enum is defined as "reads local files the operator provides / writes output to disk" and every current `local` skill stays inside the project. This module is the first to write to the user's GLOBAL `~/.claude/` config. We use `local` as the closest existing value (it is still local, keyless, no-account, no-network) and add a one-line comment in the SKILL.md frontmatter noting it writes global config, NOT project files. We deliberately do NOT add a new `data_path` enum (e.g. `config`/`local_home`) now: that is a manifest-contract change touching `manifest.py`, the registry generator, and the binding check, and YAGNI for a single skill. Revisit a dedicated enum only if more global-config-writing skills appear. Its body:
...
- **Robustness on the global CLAUDE.md write (resolves spec-review Issue 5):** unlike the directory-copy install (which tracks ownership in `bos.json`), a delimited block inside one file has no per-section ownership registry, so the tool must defend the write: (a) if the file is unreadable or not valid UTF-8, refuse and report rather than overwrite; (b) if it finds a BOS `start` marker without a matching `end` marker (a corrupted block from a failed prior run), REFUSE to write and tell the user to fix or remove that block manually, never guess the boundary; (c) only when exactly-zero or one well-formed marker pair exists does it insert (zero) or replace-in-place (one). The settings.json merge must likewise handle a malformed/invalid existing `settings.json` by refusing and reporting, never silently replacing it.

### Source content
- Settings: reuse `settings/recommended-settings.json` (the permission allow/deny set) as the source of the recommended merge. **TrustPager-specific permissions (resolves spec-review Issue 4):** that file's allow-list contains general-purpose entries (`Read`, `Glob`, `Grep`) AND TrustPager-specific patterns (`mcp__trustpager__get_*`/`list_*`/`search_*`) plus destructive denies. The general-purpose entries are merged always (every user benefits). The TrustPager-specific entries are pre-allows for safe reads that do NOTHING until the user connects TrustPager (they only match a TrustPager MCP server that is not present otherwise), so they are harmless to a keyless user. Recommended mode merges them with a one-line plain explanation ("these pre-approve safe TrustPager reads so you are not prompted once you connect it; they do nothing until then"). Guided mode asks first ("do you use or plan to use TrustPager? If so I will pre-allow its safe reads"). Never merge any write/destructive allow.
...
### Wiring + discovery
- Promote in `knowledge/starter-projects.md` and ensure `/whats-possible` surfaces it. **Binding-check note (resolves spec-review Issue 3 framing):** the onboarding-binding check has no "opt-in vs cold-win" concept; it checks that any referenced app-id exists in `kernel/registry.json` (assertion A) and is honestly tagged (assertion B). So this module must be a real, registered, keyless-tagged skill. "Opt-in" means we simply do NOT route it from `start-here` and present it as a recommended module, not that it is exempt from the registry/binding rules.
- A `commands/tune-my-setup.md` slash-command shim.
- Do NOT route it from `start-here`.

### Implementation sequencing (resolves spec-review Issue 2)
The signpost `tool` mode is guarded by a drift test (`tests/test_run_tool_mode.py::test_every_invoked_tool_is_allowlisted`) that scans `skills/`+`commands/` for `bos-run.py tool <name>` and asserts every referenced tool exists AND is in `_ALLOWED_TOOLS`. Therefore, in the SAME commit (or before) the SKILL body references `tool setup_claude_config`: (1) create `tools/setup_claude_config.py`, and (2) add `setup_claude_config` to `_ALLOWED_TOOLS` in `tools/run.py`. After adding the skill, run `python tools/registry-generator.py` to regenerate `kernel/registry.json` (and `export-capabilities.py` if a one-liner is added) so assertion A passes and the registry/CAPABILITIES freshness checks stay green.
