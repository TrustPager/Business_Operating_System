...
| 23 | A live renewal tracker .xlsx where the days-until-renewal column recalculates on open, rows soonest-first | All; trades, consulting, broking, allied health, hospitality | save-time | renewal-tracker (✅ keyless: reasoning-only + required live-formula .xlsx via write_xlsx; =DATE(yyyy,m,d)-TODAY() per row; soonest-first; status flags; connect-tier reminder split stated honestly) | ✅ |
| 24 🇦🇺 | Quarterly Simpler-BAS GST figures prepared (G1 / 1A / 1B) with the GST calc shown and the ATO rate sourced, ready to enter | Australian businesses only | get-paid (AU tax) | estimate-my-bas (✅ keyless: reasoning-only; `tools/regional.py` `load_au_constants("AU")` for the GST rate + Simpler-BAS field map; prepare-only, never lodges; optional .xlsx via write_xlsx). Region-gated: `requires_region: AU` in the manifest; reachable only when the profile explicitly sets `Region: AU`, never inferred from a city, address, language, timezone, or currency. | ✅ (AU-gated) |

**AU pack (region-gated).** Row 24 is the first Australia-only floor app. The gate holds at five layers: the manifest (`requires_region: AU`), the registry (carried through by the generator), the onboarding-binding check (assertion D: it may only be referenced inside an AU-gated section of the surface), the onboarding surface itself (start-here asks an explicit opt-in region question and only offers or routes it once `Region: AU` is set; starter-projects lists it only under the "Australian businesses only" heading with a `requires_region:au` row tag), the data loader (`load_au_constants` refuses any region other than `"AU"`), and the skill body (a refusal step that reads the profile `Region:` line and stops unless it is exactly `AU`). Region is never inferred; the explicit `Region:` profile line is the sole signal.
...
| 21 | **✅ (done) renewal-tracker** | 1 | small | reasoning_only | Live renewal tracker .xlsx; days-until-renewal column uses =DATE(yyyy,m,d)-TODAY() per row (recalculates on open); rows soonest-first; status flag formula keyed to lead-time; honest connect-tier split (keyless file tracks, CRM/calendar fires reminders). Built keyless, P5. |
