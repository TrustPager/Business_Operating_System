...
2. **Their brand, split into two things (founder-clarified 2026-07-01):** brand STRATEGY (positioning, promise, voice) is the core, high-value piece and shows up early. Brand IDENTITY (the visual kit: logo/colours/fonts in `brand.json`, via `brand-my-workspace`) is *contextual*, surfaced only when the owner is about to make things look a certain way (the content day's doorway) or on request, never a Day 1 headline. Both are shipped; the change is timing/framing, not capability. Do not point a new owner at picking fonts on Day 1.
...
**Connect-on-demand plumbing SHIPPED 2026-07-01 (founder chose the general path, not a Gmail/Calendar-only flow):** `skills/connect-a-tool/SKILL.md` + command + `knowledge/connectors.md` (the extensible, searchable connector catalog; Gmail + Google Calendar written up in full, more added by the same walkthrough). `connect-a-tool` is a friendly *guided, verified tutorial*: it sells the win, gives the exact per-app steps, does the token-frugality (D10) + privacy + credit (D4) thinking, guides the one sign-in only the owner can do (honest about the OAuth boundary; never asks for a password/token; never claims success unverified), verifies with a lightweight read, then hands off to what it unlocks. Post-challenge "find connectors that fit my business" is the catalog's browse-by-need mode. Also shipped: `tools/dogfood.py` (stages the working tree into a throwaway profile for branch dogfooding, verified it never touches the real home).

**Task C COMPLETE 2026-07-01:** `skills/set-up-a-routine/SKILL.md` + command shipped, and wired into the challenge Day-5 finale (connect-a-tool → set-up-a-routine → run it once). It builds a real routine on a connected tool (flagship: a morning brief that reads the calendar and drafts follow-ups), permission-first (D11, never hands the owner a command), drafts-only (nothing sends/changes without the owner), reversible/pausable in plain language, and runs once so they see it work. Falls back to a keyless routine (weekly review) if they would rather not connect anything. Classified connected-tier (`requires_credential: mcp`, `data_path: mcp_tools`). Day 5's "coming soon" preview is now the live flow.

**Original dependency flag (now largely resolved by the plumbing above):** Day 5 rides on connect-on-demand infrastructure. What it needed:
...
**Shell SHIPPED 2026-07-01 (Days 1-4 live, Day 5 graduation + honest "what's next"):** `skills/five-day-challenge/SKILL.md` (the orchestrator + engagement doctrine + resumable daily flow), `commands/five-day-challenge.md`, and the profile marker extended with `challenge` / `challenge_wins` / `doorways_open`. Blank-canvas brand default also shipped (neutral `brand.json` + placeholder logo/favicons) so Day 1 builds onto a clean slate, not TrustPager teal. Follow-ups: (1) Task C's live connector routine SHIPPED (see Task C below). (2) **DONE 2026-07-01** — `five-day-challenge` is now the fourth registry-bound surface in `check-onboarding-binding.py` (`extract_five_day_challenge_refs` delegates to the whats-possible existence-check, so it spans tiers: every named app must exist, without asserting keyless, so the connected Day-5 `set-up-a-routine` is fine). Two tests added (phantom in challenge fails A; a clean tier-spanning challenge passes). Real binding check green with the challenge scanned.

**Build (arc — each day = one session, one cluster, a kept artifact, a transferable operator move, a low-key doorway):**
- [ ] **Day 1 — Know your business, land your first win:** `start-here` brain-dump → business profile, then it *steers* to the best-fit first win (usually the positioning / brand-strategy brief + `research-a-competitor`; for a product/creator business it may steer to the look). Move: *context in, leverage out.* Brand here = STRATEGY, not the visual kit; visual identity is contextual (Day 2 doorway / on request), never scripted here.
