...
> **Residual studio defaults (pending Phase 2+).** The `coverage.js` and `make.js` scripts still carry two hard-coded TrustPager defaults from this studio's origin: a "title should contain TrustPager" warning and a "description should end with the TrustPager CTA" warning. Those are the studio's own legacy defaults, not the owner's rules. They are WARN-only and never block a render, so a genericised owner-brand thumbnail ships fine. Making the lint read the brand name and CTA from `brand/brand.json` is a follow-up (it lives in `scripts/`, outside the Decision 9 doc-and-JSDoc flip). Until then, treat those two warnings as advisory only.
