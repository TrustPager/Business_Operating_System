...
4. **Which hero matches this topic?** Pick the iconic visual centrepiece that carries what the video is about. A walkthrough picks the product surface it covers; a story or a tips video picks the shape that best carries its idea (the hero is not tutorial-only). There are two honest paths here, and which one you're on changes the size of the job, so say so up front:
   - **An existing hero fits, so reuse it (quick).** Browse [`src/templates/heroes/`](src/templates/heroes/) and its registry in [`heroes/index.js`](src/templates/heroes/index.js). If one already carries your topic, set `"hero": "<key>"` in `samples.json` and you're done. This is the fill-in-JSON path. Heads-up on what's in stock today: every shipped hero is a CRM/workspace surface (pipeline, contacts, forms, approvals, e-signing, reports, and so on). So the reuse path is quick when your video is about one of those surfaces.
   - **Nothing fits, so a new hero gets authored (a bigger step).** If your topic is a trade or a general tutorial (a roofing walkthrough, a "3 tools every sparky needs" video), there is likely no hero for it yet, and picking an unknown key just falls back to the generic activity card, which fails the squint test. Getting it right means authoring a genuinely new React hero component and registering it, following [`heroes/index.js`](src/templates/heroes/index.js): single outer container, thin/tall/vertical stack, bleeds off the bottom, brand colours only. That is real React work, not a JSON edit, so set that expectation with the owner before starting rather than after.
