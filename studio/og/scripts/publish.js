...
// Render an OG image PNG and upload it to YOUR TrustPager workspace's
// "OG Images" folder. After upload the API returns a hosted URL you can
// drop straight into a page's <meta property="og:image"> tag, or download
// to commit into your site's /public/og/ folder.
//
// Auth resolves from the $TRUSTPAGER_API_KEY environment variable.
//
// Usage:
//   npm run publish docs-home                render + upload (skip if exists)
//   npm run publish -- --all                 render + upload every design
//   npm run publish docs-home --replace      delete existing then re-upload
...
const DEV_SERVER = 'http://localhost:3217';
const API_BASE = 'https://api.trustpager.com/functions/v1/api/v1';
const TARGET_FOLDER = 'OG Images';
...
async function uploadOne(key) {
  ...
  const body = {
    ...
    mime_type: 'image/png',
    description: `OG image: ${samples[key].data?.headline?.split('\n')[0] || key}`,
  };
