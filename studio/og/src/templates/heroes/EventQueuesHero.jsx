...
const ACTION_ICON = {
  email: { icon: '✉', color: 'var(--brand-primary)', bg: 'color-mix(in srgb, var(--brand-primary) 16%, transparent)' },
  sms:   { icon: '💬', color: 'var(--brand-accent)', bg: 'color-mix(in srgb, var(--brand-accent) 16%, transparent)' },
  task:  { icon: '◐', color: 'var(--brand-light)', bg: 'color-mix(in srgb, var(--brand-light) 20%, transparent)' },
  exit:  { icon: '↗', color: '#94a3b8', bg: 'rgba(148,163,184,0.20)' },
};

const STATE_PILL = {
  fired:   { fg: 'var(--brand-secondary)', bg: 'color-mix(in srgb, var(--brand-secondary) 16%, transparent)',  label: '✓ FIRED' },
  running: { fg: 'var(--brand-primary-deep)', bg: 'color-mix(in srgb, var(--brand-primary) 20%, transparent)',  label: '● RUNNING NOW' },
    ...
    <div style={{ display: 'flex', gap: 12, alignItems: 'stretch' }}>
      ...
      <div style={{ width: 56, flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        {!isFirst && (
          <div style={{
            width: 2, height: 8,
            background: isFired || isRunning ? 'var(--brand-secondary)' : 'rgba(148,163,184,0.30)',
          }} />
        )}
        <div style={{
          ...
          background: isRunning
            ...
            : (isFired ? 'var(--brand-secondary)' : '#fff'),
          color: (isRunning || isFired) ? '#fff' : colors.foreground,
          fontSize: 11, fontWeight: 800,
          letterSpacing: '-0.005em',
          textAlign: 'center',
          border: !isRunning && !isFired ? '1.5px solid rgba(148,163,184,0.4)' : 'none',
          boxShadow: isRunning ? '0 4px 10px color-mix(in srgb, var(--brand-primary) 35%, transparent)' : 'none',
          flexShrink: 0,
        }}>{step.day}</div>
        {!isLast && (
          <div style={{
            ...
            background: isFired
              ? 'var(--brand-secondary)'
              : (isRunning ? 'linear-gradient(180deg, var(--brand-primary), rgba(148,163,184,0.30))' : 'rgba(148,163,184,0.30)'),
            marginTop: 6,
          }} />
        )}
      </div>
      ...
      <div style={{
        ...
        boxShadow: isRunning ? '0 4px 14px color-mix(in srgb, var(--brand-primary) 18%, transparent)' : 'none',
        marginBottom: 4,
      }}>
