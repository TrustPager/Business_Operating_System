...
const StatusPill = ({ state }) => {
  const map = {
    opened:  { fg: 'var(--brand-primary-deep)', bg: 'color-mix(in srgb, var(--brand-primary) 18%, transparent)', label: 'OPENED' },
    clicked: { fg: 'var(--brand-secondary)', bg: 'color-mix(in srgb, var(--brand-secondary) 18%, transparent)', label: 'CLICKED' },
    delivered: { fg: 'var(--brand-accent)', bg: 'color-mix(in srgb, var(--brand-accent) 18%, transparent)', label: 'DELIVERED' },
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <Stat label="DELIVERED" value={CAMPAIGN.delivered} sub={`of ${CAMPAIGN.sent} sent`} color="var(--brand-accent)" />
        <Stat label="OPENS"     value={`${openPct}%`}     sub={`${CAMPAIGN.opens}`}         color="var(--brand-primary-deep)" />
        <Stat label="CLICKS"    value={`${clickPct}%`}    sub={`${CAMPAIGN.clicks}`}        color="var(--brand-secondary)" />
