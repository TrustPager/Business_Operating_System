...
const PRIORITY = {
  high:   { color: 'var(--brand-primary)', label: 'HIGH' },
  medium: { color: 'var(--brand-accent)', label: 'MED'  },
  ...
  if (state === 'done') {
    return (
      <div style={{
        ...
        flexShrink: 0,
        boxShadow: '0 2px 5px color-mix(in srgb, var(--brand-secondary) 30%, transparent)',
      }}>✓</div>
    );
  }
...
const TaskRow = ({ t }) => {
  ...
  return (
    <div style={{
      background: inProg ? 'color-mix(in srgb, var(--brand-primary) 5%, transparent)' : '#fff',
      borderRadius: 10,
      padding: '10px 12px',
      border: inProg
        ? '1.5px solid color-mix(in srgb, var(--brand-primary) 40%, transparent)'
        : '1px solid rgba(226,232,240,0.7)',
      boxShadow: inProg ? '0 4px 12px color-mix(in srgb, var(--brand-primary) 14%, transparent)' : 'none',
